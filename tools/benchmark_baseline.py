...
from scenecollisionnet.collision_models import (
    FCLMultiSceneCollisionChecker,
    SDFObjCollisionChecker,
    SDFSceneCollisionChecker,
)
...
def benchmark(model, test_loader, iterations, out_path, vis=True):
    time_start = timer()
    ...
    for batch_idx in tqdm.trange(
        ...
        if vis:
            ...
            scene, obj, trans, rots, coll = data[3:-1]
        else:
            scene, obj, trans, rots, coll = data[:-1]
        fw_pass_start = timer()
        out = model(scene, obj, trans, rots)
        passes.append(timer() - fw_pass_start)

        preds = np.append(preds, out)
        trues = np.append(trues, coll.numpy())

        if vis:
            obj_pose = obj_pose.numpy()
            trans = trans.numpy()
            rots = rots.numpy()
            scene = scene[0, :, :3].numpy()
            obj = obj[0, :, :3].numpy()
            ...
            with imageio.get_writer(
                ...
                for i, tr, rot, pr, gt in zip(
                    np.arange(len(trans)), trans, rots, out, coll.numpy()
                ):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark a CollisionNet Baseline"
    )
    ...
    config["vis"] = args.vis
    config["iterations"] = 25 if config["vis"] else 1000

    # Create output directory for model
    out = osp.join(config["output"], config["model"]["type"])
    if osp.exists(out):
    ...
    # 1. Model
    if config["model"]["type"] == "fcl":
        model = FCLMultiSceneCollisionChecker(n_proc=n_proc)
    elif config["model"]["type"] == "sdf_obj":
        model = SDFObjCollisionChecker(
            device=config["model"]["device"], n_proc=n_proc
        )
    elif config["model"]["type"] == "sdf_scene":
        model = SDFSceneCollisionChecker(
            device=config["model"]["device"], n_proc=n_proc
        )
    else:
